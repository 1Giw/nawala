...
class Surat_masuk_model extends CI_Model
{
    ...
    public function jmlSurat($tahun)
    {
        $rekap_query = "
        SELECT
            EXTRACT(YEAR FROM t_surat_masuk.\"TGL_SURAT\") AS TAHUN,
            EXTRACT(MONTH FROM t_surat_masuk.\"TGL_SURAT\") AS BULAN,
            r_bulan.\"NM_BULAN\",
            COUNT(t_surat_masuk.\"ID_SURAT_MASUK\") AS JUMLAH
        FROM 
            t_surat_masuk
        LEFT JOIN
            r_bulan ON EXTRACT(MONTH FROM t_surat_masuk.\"TGL_SURAT\") = CAST(r_bulan.\"KD_BULAN\" AS INTEGER)
        WHERE
            EXTRACT(YEAR FROM t_surat_masuk.\"TGL_SURAT\") = '" . $tahun . "'
        GROUP BY
            EXTRACT(YEAR FROM t_surat_masuk.\"TGL_SURAT\"),
            EXTRACT(MONTH FROM t_surat_masuk.\"TGL_SURAT\"),
            r_bulan.\"NM_BULAN\"
        ORDER BY
            EXTRACT(MONTH FROM t_surat_masuk.\"TGL_SURAT\") ASC
        ";
