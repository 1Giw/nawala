...
class Surat_keluar_model extends CI_Model
{
    ...
    public function jmlSurat($tahun)
    {
        $rekap_query = "
        SELECT
            EXTRACT(YEAR FROM t_surat_keluar.\"TGL_SURAT\") AS TAHUN,
            EXTRACT(MONTH FROM t_surat_keluar.\"TGL_SURAT\") AS BULAN,
            r_bulan.\"NM_BULAN\",
            COUNT(t_surat_keluar.\"ID_SURAT_KELUAR\") AS JUMLAH
        FROM 
            t_surat_keluar
        LEFT JOIN
            r_bulan ON EXTRACT(MONTH FROM t_surat_keluar.\"TGL_SURAT\") = CAST(r_bulan.\"KD_BULAN\" AS INTEGER)
        WHERE
            EXTRACT(YEAR FROM t_surat_keluar.\"TGL_SURAT\") = '" . $tahun . "' AND 
            t_surat_keluar.\"STATUS\" = '2'
        GROUP BY
            EXTRACT(YEAR FROM t_surat_keluar.\"TGL_SURAT\"),
            EXTRACT(MONTH FROM t_surat_keluar.\"TGL_SURAT\"),
            r_bulan.\"NM_BULAN\"
        ORDER BY
            EXTRACT(MONTH FROM t_surat_keluar.\"TGL_SURAT\") ASC
        ";
