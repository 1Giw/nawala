...
class Disposisi extends CI_Controller
{
    ...
    public function add()
    {
        ...
        if ($validation->run()) {
            $Data->save();
            $this->Log_surat_masuk_model->add_log($this->Auth_model->current_user()->USERNAME, $this->input->post("id_surat_masuk"), "Membuat disposisi surat masuk");
        ...
        redirect(site_url('admin/surat_masuk/tampil/' . $this->input->post("id_surat_masuk")));
    }
    ...
    public function cetak($id)
    {
        ...
        $this->Log_surat_masuk_model->add_log($this->Auth_model->current_user()->USERNAME, $diposisi->ID_SURAT_MASUK, "Mencetak Lembar disposisi surat masuk");
    ...
    public function hapus($id_surat = null, $id_disposisi = null)
    {
        ...
        if ($this->Disposisi_model->delete($id_disposisi)) {
            $this->session->set_flashdata('gagal', 'Data Surat Masuk Berhasil di hapus');
            redirect(site_url('admin/surat_masuk/tampil/' . $id_surat));
        }
    }
